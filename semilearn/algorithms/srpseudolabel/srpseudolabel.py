    ...
    def train_step(self, x_lb, y_lb, x_ulb_w):
        ...
        with self.amp_cm():
            ...
            pseudo_label = self.call_hook("gen_ulb_targets", "PseudoLabelingHook", 
                                          logits=logits_x_ulb,
                                          use_hard_label=True)
            filtered_pseudo_labels = self.semi_reward_infer(feats_x_ulb, pseudo_label, self.it)
            ...
            if self.it > 0:
                generator_loss, rewarder_loss = self.semi_reward_train(feats_x_ulb, pseudo_label, y_lb, self.it)
