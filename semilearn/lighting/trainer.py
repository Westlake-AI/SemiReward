...
class Trainer:
    ...
    def fit(self, train_lb_loader, train_ulb_loader, eval_loader):
        ...
        for epoch in range(self.config.epoch):
            ...
            if self.algorithm.task_type == 'cls':
                if result['acc'] > self.algorithm.best_eval_metric:
                    self.algorithm.best_eval_metric = result['acc']
                    save_best = True
            else:
                if result['mse'] < self.algorithm.best_eval_metric:
                    self.algorithm.best_eval_metric = result['mse']
                    save_best = True
            if save_best:
        ...
        if self.algorithm.task_type == 'cls':
            self.logger.info("Best acc {:.4f} at epoch {:d}".format(self.algorithm.best_eval_metric, self.algorithm.best_epoch))
        else:
    ...
    def evaluate(self, data_loader, use_ema_model=False):
        ...
        if self.algorithm.task_type == 'cls':
            ...
            result_dict = {'mse': mse, 'rmse': rmse, 'mae': mae, 'mape': mape, 'r2': r2}
